...
@nb.jit(nopython=True)
def generate_data(size,charges,coods,rconvs,aconvs,cutoff_r=12.0,n_atm = 2.0,smooth_cutoff=False):
            ...
            if rij_norm!=0 and rij_norm<cutoff_r:
                ...
                for k in range(j+1, size):
                    ...
                    if rik_norm!=0 and rik_norm<cutoff_r:
                        ...
                        pref = charge
                        if smooth_cutoff:
                            fcutik = fcut(rik_norm, cutoff_r)
                            fcutjk = fcut(rkj_norm, cutoff_r)
                            pref = pref*fcutij*fcutik*fcutjk
...
def get_cmbdf(charges, coods, convs, pad=None, rcut=10.0,n_atm = 2.0, gradients=False, smooth_cutoff=False):
    """"
    ...
    if gradients:
        ...
        twob, threeb = generate_data(size,charges,coods,rconvs,aconvs,rcut,n_atm,smooth_cutoff)
        mat[:size,:nr] = einsum('ij... -> i...',twob)
        mat[:size,nr:] = einsum('ijk... -> i...',threeb)
        return mat
    
def get_cmbdf_global(charges, coods, asize,rep_size,keys, convs, rcut=10.0,n_atm = 2.0, smooth_cutoff=False):
    """
...
def generate_mbdf(nuclear_charges,coords,convs='None',alpha_list=[1.5,5.0],n_list=[3.0,5.0],gradients=False,local=True,n_jobs=-1,a1=2.0,pad=None,rstep=0.0008,rcut=10.0,astep=0.0002,nAs=4,order=4,progress_bar=False,a2=2.0,n_atm = 2.0,smooth_cutoff=False):
    ...
    if local:
        if gradients:
            if progress_bar:
                ...
                mbdf = Parallel(n_jobs=n_jobs)(delayed(get_cmbdf)(charge, cood, convs, pad, rcut,n_atm, gradients=True, smooth_cutoff=smooth_cutoff) for charge,cood in tqdm(list(zip(charges,coords))))

            else:
                mbdf = Parallel(n_jobs=n_jobs)(delayed(get_cmbdf)(charge, cood, convs, pad, rcut,n_atm, gradients=True, smooth_cutoff=smooth_cutoff) for charge,cood in list(zip(charges,coords)))
            ...
            if progress_bar:
                from tqdm import tqdm
                reps = Parallel(n_jobs=n_jobs)(delayed(get_cmbdf)(charge, cood, convs, pad, rcut,n_atm, gradients=False, smooth_cutoff=smooth_cutoff) for charge,cood in tqdm(list(zip(charges,coords))))

            else:
                reps = Parallel(n_jobs=n_jobs)(delayed(get_cmbdf)(charge, cood, convs, pad, rcut,n_atm, gradients=False, smooth_cutoff=smooth_cutoff) for charge,cood in list(zip(charges,coords)))
        ...
        if progress_bar:
            from tqdm import tqdm
            reps = Parallel(n_jobs=n_jobs)(delayed(get_cmbdf_global)(charge, cood,asize,rep_size,keys, convs, rcut,n_atm, smooth_cutoff=smooth_cutoff) for charge,cood in tqdm(list(zip(charges,coords))))

        else:
            reps = Parallel(n_jobs=n_jobs)(delayed(get_cmbdf_global)(charge, cood, asize,rep_size,keys, convs,rcut,n_atm, smooth_cutoff=smooth_cutoff) for charge,cood in list(zip(charges,coords)))
